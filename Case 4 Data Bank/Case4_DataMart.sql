...
VALUES
  ...
  --Creating the Customer Nodes Table
...
CREATE TABLE customer_transactions1 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO customer_transactions1
...
CREATE TABLE customer_transactions2 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO customer_transactions2
  (customer_id, txn_date, txn_type, txn_amount)
VALUES
  ...
  CREATE TABLE customer_transactions3 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO customer_transactions3
  (customer_id, txn_date, txn_type, txn_amount)
VALUES
  ...
  CREATE TABLE customer_transactions4 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
VALUES
  ...
  CREATE TABLE customer_transactions5 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO customer_transactions5
  (customer_id, txn_date, txn_type, txn_amount)
VALUES
  ...
  CREATE TABLE customer_transactions6 (
  customer_id INTEGER,
  txn_date DATE,
  txn_type VARCHAR(10),
  txn_amount INTEGER
...
INSERT INTO customer_transactions6
  (customer_id, txn_date, txn_type, txn_amount)
VALUES
...
INNER JOIN customernodess AS cus ON reg.region_id = cus.region_id
GROUP BY cus.region_id, reg.region_name
-- Australia has the most number of custoemrs allocated, Europe has the least number of customers allocated
...
--Because the dataset was way over 1000 rows, 
--I had to split the customer transactions data into 6 tables and UNION ALL them in a CTE to create the customer nodes table
...
--Use count to find the unique transaction types numbers and SUM to calculate the total amounts.
...
--First we create a subquery to pull the number of deposits, purchases and, and number of withdrawals for each month and you can see that
--We are using the Date Part functions to get the month and date name to get the name of the month.
...
--First we use a subquery to aggregate the customer transactions by the customer and the month they made the transaction
--Within the subquery, we will use the date add function to truncate the date column to have it start at the beginning of the month so that we can group the
--transactions by month.
--Then we will use sum to get the total transactions and within that we will distinguish the deposits and the withdrawals with a case statement
--Lastly we will use the main outer query to pull the closing balance based on the month
...
--First we will create a CTE to calculate the total transactions towards the end of the month
--Then we will create a CTE to calculate the closing balance of each customer per month and the percentage increase in the closing balance for each customer.
--Then our main outer query will calculate the percentage of customers whose closing balance increased 5% compared to the previous month
...
--Step 1 Calculating the running balance for each customer
...
--First we will create the CTE to get the total monthly transactions for each customer for each month.
--Next we will create the CTE to get the running balance and later calculate the month end balance for each customer
...
April too
also had a high data requirement which means there were also alot of data transactions happening in that month as well.
...
/*
The query's output shows the approximate data required for each customer on a monthly basis when they take an annual 6% interest rate divided by 365 days to be calculated daily
Therefore, the data allocation of each customer will grow in a similar way of how money saved up in a high yield savings account will grow from the interest.
Furthermore, the data team can use these insights to  convince customers to perform more transactions while increasing their data allocation.
*/